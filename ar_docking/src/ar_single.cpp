...
//     ROS_INFO ("\tMarker Pattern Filename: %s", pattern_filename_);
  ...
  void ARDockingPublisher::camInfoCallback (const sensor_msgs::CameraInfoConstPtr & cam_info)
  {
    if (!getCamInfo_)
    {
      cam_info_ = (*cam_info);
...
// DEPRECATED: Fuerte support ends when Hydro is released
  ...
  void ARDockingPublisher::computeCmdVel(double quat[4], double pos[3]) {
    ...
    double t = lambda_ * cmd_t + kt * (pose[0]/pose[2]);
    double x = lambda_ * cmd_x + kx * (pose[2]);

    geometry_msgs::Twist msg;
    msg.pose.pose.linear.x  = x; 
    msg.pose.pose.angular.z = t;
    vel_pub_.publish(msg);
  }


  void ARDockingPublisher::getTransformationCallback (const sensor_msgs::ImageConstPtr & image_msg)
  {
    ARUint8 *dataPtr;
